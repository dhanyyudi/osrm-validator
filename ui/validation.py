...
def setup_osrm_parameters():
    ...
    with st.expander("OSRM API Parameters", expanded=True):
        ...
        with col2:
            ...
            st.write("**Start Time Configuration**")
            ...
            if start_time_option == "custom":
                ...
                time_offset_hours = st.number_input(
                    ...
                    max_value=24,
                    value=0,
                    help="Offset in hours from current time"
                )
            ...
            st.text_input(
                ...
                disabled=True,
                help="This will be used as start_time parameter in OSRM requests"
            )
        ...
        # Pre-populate with start_time example if empty
        if not custom_params:
            custom_params = {"start_time": "2024-08-22T10:00:00+00:00"}
        ...
        edited_custom_params = st.data_editor(
            ...
            column_config={
                "Parameter": st.column_config.TextColumn(
                    "Parameter Name",
                    help="Custom parameter name (e.g., 'start_time', 'alternatives', 'continue_straight')"
                ),
                "Value": st.column_config.TextColumn(
                    "Parameter Value",
                    help="Parameter value (for start_time use format: 2024-08-22T10:00:00+00:00)"
                )
            }
        )
        
        # Helper buttons for start_time
        col1, col2, col3 = st.columns(3)
        
        with col1:
            if st.button("Add Current Time as start_time"):
                current_time = datetime.now().strftime('%Y-%m-%dT%H:%M:%S+00:00')
                st.info(f"Add this to Custom Parameters: start_time = {current_time}")
        
        with col2:
            if st.button("Add Custom Time as start_time"):
                custom_datetime = datetime.now() + timedelta(hours=1)  # Default 1 hour from now
                custom_time_str = custom_datetime.strftime('%Y-%m-%dT%H:%M:%S+00:00')
                st.info(f"Add this to Custom Parameters: start_time = {custom_time_str}")
        
        with col3:
            time_offset = st.number_input("Hours offset", value=0, min_value=-24, max_value=24)
            if st.button("Apply Offset"):
                offset_datetime = datetime.now() + timedelta(hours=time_offset)
                offset_time_str = offset_datetime.strftime('%Y-%m-%dT%H:%M:%S+00:00')
                st.info(f"Add this to Custom Parameters: start_time = {offset_time_str}")
