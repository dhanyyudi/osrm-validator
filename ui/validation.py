...
def setup_osrm_parameters():
    ...
    # Initialize session state for start time persistence
    if "start_time_option" not in st.session_state:
        st.session_state.start_time_option = "Use Current Time"
    ...
    with st.expander("OSRM API Parameters", expanded=True):
        ...
        # Start Time Parameter - FIXED PERSISTENCE
        ...
        start_time_choice = st.radio(
            ...
            index=["Use Current Time", "Use Custom Time", "Add to Custom Parameters"].index(st.session_state.start_time_option),
            key="start_time_choice_radio"
        )
        
        # Update session state
        st.session_state.start_time_option = start_time_choice
        ...
        if start_time_choice == "Use Custom Time":
            ...
            # Show helper for current values
            helper_datetime = datetime.combine(st.session_state.custom_start_date, st.session_state.custom_start_time)
            helper_value = helper_datetime.strftime('%Y-%m-%dT%H:%M:%S+00:00')
            st.code(f"Example: start_time = {helper_value}")
        ...
        # Custom parameters section (ORIGINAL)
        ...
        if st.button("Save OSRM API Configuration"):
            ...
            # Add start_time if configured (MINIMAL ADDITION)
            if start_time_choice == "Use Custom Time" and 'start_time_value' in locals():
                new_settings["START_TIME"] = start_time_value
            elif start_time_choice == "Use Current Time":
                new_settings["START_TIME"] = datetime.now().strftime('%Y-%m-%dT%H:%M:%S+00:00')
            ...
            # Show confirmation of saved start_time
            if "START_TIME" in new_settings:
                st.info(f"Start time saved: {new_settings['START_TIME']}")